<?php
	...
	$query = 'SELECT p.id, p.lastName as "LastName", p.FirstName, p.jobTitle as JobTitle, p.email as Email, d.name as Department, l.name as Location FROM personnel p LEFT JOIN department d ON (d.id = p.departmentID) LEFT JOIN location l ON (l.id = d.locationID) ORDER BY p.firstName, p.lastName, d.name, l.name';
